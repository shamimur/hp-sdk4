...
/rest/v1/.../collection/1, or 2.  On Moonshot a System collection member might
be /rest/v1/Systems/C1N1. This sounds very complicated, but in reality (as
    ...
    while status < 300:
        ...
        # else walk the member links
            ...
            for memberuri in thecollection['links']['Member']:
                ...
                # link
                status, headers, member = rest_get(restobj, memberuri['href'],
                                                   request_headers)
                # Read up on Python generator functions to understand what this
                # does.
        ...
        if 'links' in thecollection and 'NextPage' in thecollection['links']:
            next_link_uri = (collection_uri + '?page=' +
                             str(thecollection['links']['NextPage']['page']))
            status, headers, thecollection = rest_get(restobj, next_link_uri,
                                                      request_headers)
...
# return the type of an object (down to the major version, skipping minor,
# and errata)
def get_type(obj):
    typever = obj['Type']
...
# Build a list of decoded messages from the extended_error using
# the message registries
# An ExtendedError JSON object is a response from the with its own schema.
# This function knows how to parse the ExtendedError object and, using any
# loaded message registries, render an array of plain language strings that
# represent the response.
def render_ext_error_msg_list(extended_error):
    messages = []
    if isinstance(extended_error, dict):
        if ('Type' in extended_error and
            ...
            for msg in extended_error['Messages']:
                ...
                tmp = message_id.split('.')
                registry = tmp[0]
                msgkey = tmp[len(tmp) - 1]
                ...
                if registry in MESSAGE_REGISTRIES:
                    if (registry in MESSAGE_REGISTRIES and msgkey in
                        ...
                        msg_dict = MESSAGE_REGISTRIES[registry][
                            'Messages'][msgkey]
                        ...
                        for argn in range(0, msg_dict['NumberOfArgs']):
                            ...
                            msg_str = msg_str.replace(
                                subst, str(msg['MessageArgs'][argn]))

                        if ('Resolution' in msg_dict and msg_dict['Resolution']
                                != 'None'):
                            msg_str += '  ' + msg_dict['Resolution']

                        messages.append(msg_str)
                else:   # no message registry, simply return the msg object in
                        # string form
...
# Print a list of decoded messages from the extended_error using the message
# registries
def print_extended_error(extended_error):
    messages = render_ext_error_msg_list(extended_error)
    msgcnt = 0
    for msg in messages:
        print '\t' + msg
        msgcnt += 1
...
def create_object(host, login_account, login_password):
    restobj = rest_client(base_url=host,
...
def ex6_change_temporary_boot_order(restobj, host, boottarget, login_account,
    ...
    for status, headers, system, memberuri in collection(restobj,
        ...
        # hint:  Don't limit to version 0 here as we will rev to 1.0 at some
        #        point hopefully with minimal changes
        assert (get_type(system) == 'ComputerSystem.0' or get_type(system) ==
                'ComputerSystem.1')
        ...
        if boottarget in system['Boot']['BootSourceOverrideSupported']:
            ...
            print ('\tBootSourceOverrideTarget value "' + boottarget +
...
def ex7_find_ilo_mac_address(restobj, host, login_account, login_password):
    ...
    for status, headers, manager, memberuri in collection(restobj,
        ...
        for status, headers, nic, memberuri in collection(
                restobj, host, manager['links']['EthernetNICs']['href'], None,
...
def ex8_add_ilo_user_account(restobj, host, login_account, login_password,
    ...
    status, headers, obj = rest_get(restobj, '/rest/v1/AccountService', None)
    assert status == 200
...
def ex14_computer_details(restobj, host, login_account, login_password):
    ...
    for status, headers, system, memberuri in collection(restobj, host,
        ...
        print ('\tMemory:  ' + str(system['Memory']['TotalSystemMemoryGB']) +
               ' GB')
        print ('\tProcessors:  ' + str(system['Processors']['Count']) + ' x ' +
               str(system['Processors']['ProcessorFamily']))
...
def ex15_mount_virtual_media_iso(restobj, host, login_account, login_password,
    ...
    for status, headers, manager, memberuri in collection(restobj, host,
        ...
        for status, headers, virtual_media, memberuri in collection(
                restobj, host, manager['links']['VirtualMedia']['href'], None,
                login_account, login_password):
...
def ex24_configure_snmp(restobj, host, login_account, login_password,
    ...
    for status, headers, manager, memberuri in collection(restobj, host,
        ...
        status, headers, network_service = rest_get(
            restobj, manager['links']['NetworkService']['href'], None)
        assert (get_type(network_service) == 'ManagerNetworkService.0' or
                get_type(network_service) == 'ManagerNetworkService.1')
        ...
        status, headers, snmp_service = rest_get(
            restobj, network_service['links']['SNMPService']['href'], None)
        ...
        print ('PATCH ' + json.dumps(config) + ' to ' +
               network_service['links']['SNMPService']['href'])
        status, headers, response = rest_patch(
            restobj, network_service['links']['SNMPService']['href'], config)
        print_extended_error(response)
        assert status == 200
...
def ex26_get_registry(restobj, host, login_account, login_password,
    ...
    for status, headers, registryptr, memberuri in collection(
            restobj, host, '/rest/v1/Registries', None, login_account,
            login_password):
...
def ex27_set_ilo_timezone(restobj, host, olson_timezone, login_account,
    ...
    for status, headers, manager, memberuri in collection(restobj, host,
        ...
        status, headers, datetime = rest_get(
            restobj, manager['Oem']['Hp']['links']['DateTimeService']['href'],
            None)
        ...
        for timezone in datetime['TimeZoneList']:
            if timezone['Name'].startswith(olson_timezone):
                ...
                print (
                    ...
                    manager['Oem']['Hp']['links']['DateTimeService']['href'])
                status, headers, response = rest_patch(
                    restobj,
                    manager['Oem']['Hp']['links']['DateTimeService']['href'],
                    request)
...
def ex29_get_powermetrics_average(restobj, host, login_account,
    ...
    # https://<host>/rest/v1/Chassis/{item}/PowerMetrics#/PowerMetrics/
    ...
    for status, headers, chassis, memberuri in collection(restobj, host,
        ...
        status, headers, pwrmetric = rest_get(
            restobj, chassis['links']['PowerMetrics']['href'], None)

        # verify expected type
        # hint:  Don't limit to version 0 here as we will rev to 1.0 at some
        #        point hopefully with minimal changes
        assert (get_type(pwrmetric) == 'PowerMetrics.0' or
                get_type(pwrmetric) == 'PowerMetrics.1')
        ...
        if ('PowerMetrics' not in pwrmetric or
            ...
            print ('\tPowerMetrics resource does not contain '
                   '"AverageConsumedWatts" or "IntervalInMin" property')
        else:
            print ('\t' + chassis['Model'] + ' AverageConsumedWatts = ' +
                   str(pwrmetric['PowerMetrics']['AverageConsumedWatts']) +
                   ' watts over a ' +
                   str(pwrmetric['PowerMetrics']['IntervalInMin']) + ' minute'
                   ' moving average')
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler("HpRestfulApiExamples.log")
