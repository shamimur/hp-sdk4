...
from hpsdk.rest.v1_helper import (RisObject)
...
class ValidationManager(object):
    """ Keep track of all the schemas and registries and provides helpers to
    simplify validation """
                ...
                if not os.path.isdir(defaultbiospath):
                    ...
                    if bioszip and os.path.exists(bioszip):
                        ...
                        raise SchemaValidationError(
                            ...
                            ' zip file found.\nPlease refer to '
                            'our documentation for further '
                            'instructions on downloading the '
                            'appropriate schemas.')
                else:
    ...
    def add_location(self, schema_path=None, registry_path=None,
        ...
        """
        Add schema_path and registry_path to the list of locations to search
        for schemas and registries.

        :param schema_path/registry_path: directory or URL where
               schemas/registries are located.
        :type  schema_path/registry_path: str.
        :raises ValueError: if schema_path/registry_path is None
                            or empty string
        """
        if schema_path:
            if not biossection:
        ...
        for location in locationslist:
            if monolith:
                ...
                for root, _, filenames in os.walk(os.path.join(location,
                                                               pathjoinstr)):
                    for filename in filenames:
                        fqpath = os.path.abspath(
                            os.path.join(os.path.normpath(root), filename))
    ...
    def new_load_file(self, monolith, root=None, biossection=False,
        ...
        for itemtype in monolith.types:
            if ((itemtype.startswith("#SchemaFileCollection.") or \
                    itemtype.startswith("Collection.")) and u'Instances' in \
                    monolith.types[itemtype]):
                for instance in monolith.types[itemtype][u'Instances']:
                    if (self._schemaid[0] in
                            instance.resp.request.path.lower() or
                            self._regid[0] in
                        ...
                        if (not registries and self._schemaid[0] in
                            ...
                            if classesdataholder:
                                if self._schemaid[1] in instance.resp.dict:
                                    classesdataholder[0][self._schemaid[1]].extend(\
                                                instance.resp.dict[self._schemaid[1]])
                            else:
                            ...
                            if classesdataholder:
                                if monolith.is_redfish:
                                    classesdataholder[0][self._regid[1]].extend(\
                                                    instance.resp.dict[self._regid[1]])
                            else:
                                classesdataholder.append(instance.resp.dict)
        ...
        try:
            if monolith._typestring in classesdataholder and ('Collection.' in \
                    classesdataholder[monolith._typestring] or ('#SchemaFileCollection.' in \
                    classesdataholder[monolith._typestring] and monolith.is_redfish)):
    ...
    def validate(self, item, selector=None, currdict=None, monolith=None,
        ...
        """
        Search for matching schemas and attribute registries and ensure that
        item is valid.
        """
        if regloc:
    ...
    def _is_local(self, path):
        ...
        :type path: str.
        :returns: False if path contains '://' True otherwise
        :rtype: boolean
        """
        if u'://' in path:
            return False
...
class Classes(RisObject):
    ...
    def find_bios_registry(self, regname):
        ...
        if hasattr(self, 'Items') and isinstance(self.Items, list):
            for entry in self.Items:
                if (u'Schema' in entry and regname.lower() in
...
class RepoRegistryEntry(RepoBaseEntry):
    ...
    def validate(self, tdict, errlist=None, selector=None, currdict=None,
        ...
        :type tdict: list.
        :param rootpath: path used to reconstruct full paths.
        :type rootpath: str.
        :returns: list a validation errors, ( emtpy if not errors )
        :rtype: list.
        """
        if not errlist:
            errlist = list()
        ...
        elif checkall and selector == None:
            results = reg.validate_attribute_values(tdict)
            errlist.extend(results)
        else:
            errlist.append(RegistryValidationError(
                u'Unable to locate registry model'))
    ...
    def validate_bios_version(self, tdict, errlist=None, selector=None,
        ...
        :param rootpath: path used to reconstruct full paths.
        :type rootpath: str.
        :returns: list a validation errors, ( empty if not errors )
        :rtype: list.
        """
        if not errlist:
            errlist = list()
        ...
        if reg and not checkall:
            for item in reg.Attributes:
                ...
                if item["ReadOnly"] == True:
                    return 'readonly'
                try:
                    if item["IsSystemUniqueProperty"] == True:
                        return 'unique'
                except BaseException:
                    continue
                else:
    ...
    def get_registry_model(self, currdict=None, monolith=None, errlist=None,
                           skipcommit=False, searchtype=None, newarg=None,
                           latestschema=None):
        """
                    ...
                    for instance in monolith.types[itemtype][u'Instances']:
                        try:
                            ...
                            if latestschema:
                                ...
                                if (currtype == insttype or
                                        currtype == instance.resp.dict[
                                            "oldtitle"].split('.')[:1]):
                                    location_file = instance.resp.dict
                                    break
                            elif (searchtype == "ob" and instance.resp.dict\
                                  ["title"].startswith(currtype) or ("oldtitle" in\
                                     instance.resp.dict.keys() and (currdict\
                                    [instance._typestring] == instance.resp.dict\
                                                                ["oldtitle"]))):
                                location_file = instance.resp.dict
                                break
                            elif (not searchtype == "ob" and currdict[instance._typestring]
                            ...
                            if ('properties' in regcopy[arg].iterkeys()
                                ...
                                for pattern in regcopy.iterkeys():
                                    ...
                                    for item in oneof:
                                        ...
                                        if not arg == newarg[-1]:
                                            try:
                                                next=newarg[newarg.index(arg)+1]
                                                regcopy[next]
                                                break
                                            except:
                                                continue
...
class RepoSchemaEntry(RepoBaseEntry):
    ...
    def validate(self, tdict, errlist=None):
        ...
        if not location_file:
            ...
            jsonschema.validate(tdict, jsonsch)
...
class HpPropertiesRegistry(RisObject):
    ...
    def validate_attribute_values(self, tdict):
        ...
        :returns: list of errors encountred.
        :rtype: list.
        """
        result = list()
...
class BaseValidator(RisObject):
    ...
    def validate(self):
        ...
        raise RuntimeError(
            u'You must override this method in your derived class')
...
class BoolValidator(BaseValidator):
    ...
    def validate(self, newval, name):
        """ Validate against ilo schema """
        result = list()
        if newval == False or newval == True:
            return result
    ...
    def validate(self, newval, name):
        ...
        if u'LowerBound' in self:
            if intval < int(self[u'LowerBound']):
                result.append(RegistryValidationError(
                    u"'%s' must be greater than or equal to '%s'" %
                    (self.Name, int(self[u'LowerBound'])), regentry=self))

        if u'UpperBound' in self:
            if intval > int(self[u'UpperBound']):
                result.append(RegistryValidationError(
                    u"'%s' must be less than or equal to '%s'" %
                    (self.Name, int(self[u'LowerBound'])), regentry=self))
